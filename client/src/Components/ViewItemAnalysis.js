...
class ViewItemAnalysis extends Component{
  ...
  constructor(props){
    ...
    this.state = {
      ...
      itemanalysis_id: props.location.state.itemanalysis_id,
      subject: "",
      filename: "Untitled",
      total_rawscore: 0,
  ...
  itemHandler(e) {
    var val = parseInt(e.target.value);
    if(val > this.state.total_num_of_students){
      e.preventDefault();
      alert("The number must not exceed the total number of students.");
    }else{
  ...
  viewItemanalysis(){
    ...
    axios.post('http://localhost:3001/itemanalysis/view-itemanalysis', body)
        .then(res =>{
            ...
            this.setState({
              ...
              total_rawscore: datum.total_rawscore,
              total_num_of_students: datum.total_num_of_students,
              number_of_items: datum.number_of_items,
              mean: datum.mean,
              students_above_mean: datum.students_above_mean,
              students_equals_mean: datum.students_equals_mean,
              students_below_mean: datum.students_below_mean,
              year: datum.year,
              section: datum.section
            })
            let total = datum.number_of_items;
            var item_froe1 = Array.from('0'.repeat(total));
            var foe_rank1 = Array.from(" ".repeat(total));
            var ns_rawscore1 = Array.from('0'.repeat(total+1));
            var prod1 = Array.from('0'.repeat(total+1));
            for (let i = 0; i < total; i++) {
              ...
              axios.post('http://localhost:3001/foe_rawscore/view-foe', body2)
                .then(res =>{
                  if(res.status === 200){
                    let datum2 = res.data.data;
                    console.log(datum2);
                    item_froe1[i] = datum2.foe;
                    if(datum2.rank !== null){
                      foe_rank1[i] = datum2.rank;
                    }
                    ns_rawscore1[rscore] = datum2.nswgrs;
                    prod1[rscore] = datum2.product;
                  }
                }
                ).catch(err =>{
                    console.log(err)
            ...
            axios.post('http://localhost:3001/foe_rawscore/view-foe', body2)
              .then(res =>{
                if(res.status === 200){
                  let datum2 = res.data.data;
                  console.log(datum2);
                  ns_rawscore1[0] = datum2.nswgrs;
                  prod1[0] = datum2.product;
                }
              }
              ).catch(err =>{
                  console.log(err)
            ...
            this.setState({
              item_foe: item_froe1,
              num_of_studs_rawscore: ns_rawscore1,
              product: prod1,
              rank: foe_rank1
            })
  ...
  createItems(){
    ...
    for (var i = 0; i < num_items; i++) {
      ...
      table_d.push( 
          <tr>
            <td >{i+1} </td>
            <td> <input type="number" min="0" max={this.state.total_num_of_students} onKeyPress={this.keypressHandler} onChange={this.itemHandler}></input> </td>
            <td>{this.state.rank[i]}</td>
            <td>{num_items-i} </td>
            <td> <input type="number" min="0" max={this.state.total_num_of_students} onKeyPress={this.keypressHandler} onChange={this.scoreHandler}></input> </td>
  ...
  render(){
    return(
      <div>
      <div class="feed"> 
      <button class="iconButton"><Link to="/"><FcHome size="30"/></Link></button>
        <table>
        <tbody>
        <tr>
        </tr>
        ...
        <div>
          ...
          <tbody>
            <tr>
            <td> <input type="text" onChange={this.filenameHandler} value={this.state.filename} placeholder={"Untitled"}></input> </td>
            </tr>
            <tr>
            <td ><b>Number of Items</b> <input type="number" min="0" onKeyPress={this.keypressHandler} onChange={this.numitemsHandler} default={10} value={this.state.number_of_items} placeholder={10}></input> </td>
              <td ><b>Total number of students</b> <input type="number" min="0" onKeyPress={this.keypressHandler} value={this.state.total_num_of_students} onChange={this.totalHandler}></input> </td>
              <td><b>Year Level</b><Select id="selectButton" onChange={this.yearHandler} value={this.state.year} options={yearOptions} placeholder={this.state.year} /> </td>
              <td> <b>Section</b> <input type="text" onChange={this.sectionHandler} value={this.state.section} placeholder={"Section"}></input> </td>
              <td><b>Subject</b><Select id="selectButton" onChange={this.subjectHandler} options={subjectOptions} value={this.state.subject} placeholder={this.state.subject} /> </td>
            </tr>
            </tbody>
            </table>
            <table>
            ...
            <tr>
              ...
              <td id="stick"> <button onClick={this.saveToDatabase}> <CSVLink data={data} headers={headers} filename={this.state.filename +".csv"} onClick={this.arrangeData}>
...
export default ViewItemAnalysis;